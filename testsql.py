...
class CreateXml() :
    ...
    jsonFilename = 'c:/Users/ilya.chuiko/Desktop/products.json'
    ...
    def avito(self) -> None :
        ...
        exportFilename = 'c://Users//ilya.chuiko//Desktop//avitoExport.xml'
...
class Parser() :
    ...
    descriptionText = 'В объявлении указан наименьший размер! Остальные размеры также в наличии ! ЗАКАЗЫ ОТПРАВЛЯЕМ ПОЧТОЙ РОССИИ НАЛОЖЕННЫМ ПЛАТЕЖОМ, ОПЛАТА ПОСЛЕ ПРИМЕРКИ! Если вы не нашли ваш размер, напишите нам, мы подберем подходящий или предложим другой вариант кроссовок! Также у нас есть скидки постоянным покупаетлям и сезонные акции!'
    additionalPrice = 700

    #txtFilename = '/Users/ilyachuiko/Desktop/dev/sneakershop/links.txt'
    txtFilename = 'c://Users//ilya.chuiko/Desktop//links.txt'
    ...
    def createDescription(self, url:str, sizes:list) -> str :
    ...
    def getInfoFromPage(self, url:str) -> dict :
        ...
        productDescription = self.createDescription(url,productSizes)

        brand = soup.find('div',class_='description')
        productBrand = brand.find('a').text
        
        labels = soup.find_all('div', class_='attributes_name')
        model = labels[-2]
        productModel = model.find('label').text
        ...
        productInfo = {
            ...
            'brand' : productBrand, #str
            'model' : productModel, #str
        }
...
aww.getInfoFromPage('https://bizoutmax.ru/index.php?route=product/product&path=1789_1_117&product_id=24554')
#aww.createDescription('https://bizoutmax.ru/index.php?route=product/product&path=1789_1_117&product_id=24554')
